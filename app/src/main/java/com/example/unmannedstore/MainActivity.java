    ...
    public void login() {
        ...
        query.findObjects(new FindListener<User>() {
            ...
            public void done(List<User> list, BmobException e) {
                if (null != e) {
                    ShowUtils.show(MainActivity.this, e.getMessage());
                } else if (list.size() == 0) {
                    ShowUtils.show(MainActivity.this, "用户名或密码错误");
                } else {
                    //登陆成功跳转
                    // ShowUtils.out(list.get(0).toString());
                    user = list.get(0);
                    int day = Integer.parseInt(user.getUpdatedAt().split(" ")[0].split("-")[2]);
                    if (new Date().getDate() - day >= 1) {
                        ShowUtils.log("健康过期");
                        user.setHealth(false);
                    }
                    ShowUtils.show(MainActivity.this, "登录成功");
